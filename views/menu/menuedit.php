...
<div class="tabcones">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
            ...
            <td>
                <select name="parentid" style="width:220px" <?php if(!empty($menus[$infos['menuid']])) {echo 'disabled';} ?>>
                    <option value="0" <?php if(!empty($menus[$infos['parentid']])) {echo 'disabled';} ?>>顶级菜单</option>
                    <?php foreach ($menus as $item){ ?>
                        <option value="<?=$item['menuid']?>"<?php if($infos['parentid']==$item['menuid']) {echo 'selected';} ?>
                            <?php if($infos['menuid']==$item['menuid']) {echo 'disabled';} ?>>
                            <?=$item['title']?><?=$item['codepath']?>
                        </option>
                    <?php }?>
                </select>
            </td></tr>
            <td class="txtlft"><span style="color:red;"> *&nbsp;</span>排序号：</td>
            <td><input type="text" value="<?=$infos['displayorder']?>" name="displayorder" class="inp a40 displayorder" onblur="checkdisplayorder()" /><em id="displayorder_msg"></em></td>
        </tr>
        <tr>
            <td class="txtlft"><span style="color:red;"> *&nbsp;</span>CODE标识：</td>
            <td><input type="text" value="<?=$infos['code']?>" name="code" class="inp a40 code" onblur="checkcode()" /><em id="code_msg"></em></td>
        </tr>
        <tr>
            <td class="txtlft"><span style="color:red;"> *&nbsp;</span>路径(URL)：</td>
            <td><input type="text" value="<?=$infos['codepath']?>" name="codepath" class="inp a40 codepath" onblur="checkcodepath()" /><em id="codepath_msg"></em></td>
        </tr>
        <tr>
            <td class="txtlft"><span style="color:red;"> *&nbsp;</span>菜单名称：</td>
            <td><input type="text" value="<?=$infos['title']?>" name="title" class="inp a40 title" onblur="checktitle()" /><em id="title_msg"></em></td>
        </tr>
     </table>
</div>
<div style="text-align:center;margin-top:10px"><input type="button"  value="修改" class="combtn cbtn_4 formsbt" /></div>
</form>
    ...
    function checkcode() {
        ...
        if(code==''){
            ...
            $.ajax({
                ...
                dataType:'json',
                data:{name:'code',namevalue:code,nameold:'codeold',nameoldvalue:"<?=$infos['code']?>"},
                async:false,
    ...
    function checkcodepath() {
        ...
        if(codepath==''){
            ...
            $.ajax({
                ...
                url:"<?=geturl('menu/isnameexist')?>",
                dataType:'json',
                data:{name:'codepath',namevalue:codepath,nameold:'codepathold',nameoldvalue:"<?=$infos['codepath']?>"},
                async:false,
                success:function(data){
                    console.log(data);
                    if(data.status === 0){
