...
<script type="text/javascript">
        function ChangeTab(obj, cat) {
            location.href = '/data/review.html?cat='+cat;
        }
</script>
<form name="form" id="form" action="/data/review.html" method="get">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="word-break:break-all;">
  <tr class="tabtit">
	 ...
	 <td class="leftxt" width="15%">评论内容</td>
	 <td width="10%">账号</td>
	 <td width="5%">评论人</td>
	 <td width="5%">评论对象</td>
...
<?php foreach($reviews as $key=>$review){?>
    ...
    <td>
    <?php if(empty($review['admin_status'])||$review['admin_status']==3){?>
    	<input type="checkbox" value="<?php echo $review['logid']?>" name="sel" />
	<?php }?>
	<?php echo max(0,($request['page']-1))*$request['pagesize']+$key+1?></td>

    <td class="leftxt"><a class="Jview" href="/data/view.html?logid=<?php echo $review['logid']?>" title="评论详情" style="text-decoration:underline;color:#075587" ><?php echo $review['subject']?></a></td>
    <td><?php echo $review['username']?></td>
    <td><?php echo $review['realname']?></td>
	<td><?php echo $review['coursewaretitle']?></td>
	<td class="leftxt"><?php echo $review['crname']?></td>
	<td><?php echo $review['score']?>(分)</td>
	<td><?php echo $review['fromip']?></td>
	<td><?php echo date("Y-m-d H:i:s",$review['dateline'])?></td>
    <td><?php echo $review['checkname']?$review['checkname']:'--'?></td>
	<td><?php $astatus=$review['admin_status'];if($astatus==1){echo '已通过';}elseif($astatus==2){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
	<a class="Jview" href="/data/view.html?logid=<?php echo $review['logid']?>" title="评论详情">详情</a>
	<?php if($review['admin_status']==0||$review['admin_status']==3){?>
	<a class="Jcheck" toid="<?php echo $review['logid']?>">审核</a>
	<?php }else{?>
        <a class="revoke" toid="<?php echo $review['logid']?>" status="<?php echo $review['admin_status']?>">撤销</a>
        <?php }?>
	</td>
...
//                    },2000)
    ...
    $(document).on("click", ".revoke", function () {
        ...
        var cat = <?php echo $cat;?>;
        $.post('/data/revoke.html', {toid: toid, status: status, type: 3}, function (data) {
            if(cat == -1){
               if (data.code == 0) {
                    $.showmessage({
                        img: 'success',
    ...
    $(document).on("click", ".Jdel", function () {
        ...
        if (confirm("确定要删除该课件吗?")) {
            $.post('/data/delprocess.html', {toid: toid, type: 3}, function () {
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckprocess.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark,type:3},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
