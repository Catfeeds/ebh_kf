...
<form name="form" id="form" action="/data/redeem.html" method="get">
...
<option value="5" <?php echo ($request['admin_status']==5)?"selected":""?>>待审核</option>
</select>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr class="tabtit">
	 ...
	 <td class="leftxt" width="15%">兑换码名称</td>
	 <td class="leftxt" width="10%">所属网校</td>
	 <td width="10%">兑换码发布日期</td>
	 <td>课程售价</td>
	 <td>兑换码售价</td>
     <td width="5%">购买数量</td>
     <td width="5%">交易金额</td>
...
<?php foreach($redeems as $key=>$redeem){?>
    <tr class="<?php echo ($key%2==0)?"tabbg":""?>">
    <td>
	<?php if(empty($redeem['admin_status'])||$redeem['admin_status']==3){?>
    	<input type="checkbox" value="<?php echo $redeem['lotid']?>" name="sel" />
	<?php }?>
	<?php echo max(0,($request['page']-1))*$request['pagesize']+$key+1?></td>
    <td class="leftxt" title="<?php echo $redeem['name']?>">
    <?php  echo $redeem['name'] ;?></td>
	<td class="leftxt"><?php echo $redeem['crname']?></td>
    <td><?php echo date("Y-m-d",$redeem['effecttime'])?></td>
	<td><?php echo $redeem['fprice']?></td>
	<td><?php echo $redeem['price']?></td>
	<td><?php echo $redeem['number'];?></td>
    <td><?php echo $redeem['number']*$redeem['price'];?></td>
    <td><?php echo date("Y-m-d",$redeem['dateline'])?></td>
    <td><?php echo $redeem['checkname'];?></td>
	<td><?php $astatus=$redeem['admin_status'];if($astatus==1){echo '已通过';}elseif($astatus==2){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
	<?php if($redeem['admin_status']==0||$redeem['admin_status']==3){?>
	<a class="Jcheck" toid="<?php echo $redeem['lotid']?>">审核</a>
    <?php }else{?>
      --
    <?php }?>
	</td>
        ...
        dialog.button([{
            ...
            callback: function () {
                ...
                $.post('/data/checkprocess.html', {
                    ...
                    var revoke = "<a class=\"revoke\" toid=" + toid + " status=" + admin_status + ">--</a>";
                    if (data.msg) {
                        //alert(data.msg);
                        $.showmessage({
                            img: 'success',
...
//                    },2000)
    ...
    $(document).on("click", ".Jdel", function () {
        ...
        if (confirm("确定要删除吗?")) {
            $.post('/data/delprocess.html', {toid: toid, type: 15}, function () {
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckprocess.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark,type:15},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
