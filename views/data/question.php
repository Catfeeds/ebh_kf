...
<script type="text/javascript">
        function ChangeTab(obj, cat) {
            location.href = '/data/question.html?cat='+cat;
        }
</script>
<form name="form" id="form" action="/data/question.html" method="get">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr class="tabtit">
	 ...
	 <td width="17%" class="leftxt" >标题</td>
	 <td width="17%" class="leftxt" >内容</td>
	 <td class="leftxt" width="17%">所属网校</td>
	 <td>账号</td>
	 <td>提问者</td>
	 <td>时间</td>
...
<?php foreach($questions as $key=>$question){?>
    ...
    <td>
	<?php if(empty($question['admin_status'])||$question['admin_status']==3){?>
    	<input type="checkbox" value="<?php echo $question['qid']?>" name="sel" />
	<?php }?>
	<?php echo max(0,($request['page']-1))*$request['pagesize']+$key+1?></td>
    <td  class="leftxt" title="<?php echo strip_tags($question['title'])?>"><a class="Jview" href="/data/view.html?qid=<?php echo $question['qid']?>" title="答疑详情" style="text-decoration:underline;color:#075587"><?php  echo $question['title'];?></a></td>
    <?php $message =$question['message']; ?>
	<td class="leftxt" title="<?php echo strip_tags($question['message'])?>"><?php  echo $message ;?></td>
	<td class="leftxt"><?php echo $question['crname']?></td>
	<td><?php echo $question['username']?></td>
	<td><?php echo $question['realname']?></td>
	<td><?php echo date("Y-m-d H:i:s",$question['dateline'])?></td>
    <td><?php echo $question['checkname'] ? $question['checkname'] : '--'?></td>
	<td><?php $astatus=$question['admin_status'];if($astatus==1){echo '已通过';}elseif($astatus==2){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
	<a class="Jview" href="/data/view.html?qid=<?php echo $question['qid']?>" title="答疑详情">详情</a>
	<?php if($question['admin_status']==0||$question['admin_status']==3){?>
	<a class="Jcheck" toid="<?php echo $question['qid']?>">审核</a>
    <?php }else{?>
        <a class="revoke" toid="<?php echo $question['qid']?>" status="<?php echo $question['admin_status']?>" >撤销</a>
    <?php }?>
	</td>
...
//                    },2000)
    ...
    $(document).on("click", ".revoke", function () {
        ...
        var cat = <?php echo $cat;?>;
        $.post('/data/revoke.html', {toid: toid, status: status, type: 4}, function (data) {
            if(cat == -1){
              if (data.code == 0) {
                    $.showmessage({
                        img: 'success',
    ...
    $(document).on("click", ".Jdel", function () {
        ...
        if (confirm("确定要删除该课件吗?")) {
            $.post('/data/delprocess.html', {toid: toid, type: 4}, function () {
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckprocess.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark,type:4},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
