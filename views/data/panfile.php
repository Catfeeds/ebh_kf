...
<div class="tit">
<h3><a href="" style="color: #ebad06;">云盘审核</a> >> <span id="barname"><?php echo $statusArr[$request['cat']]?></span></h3>
</div>
...
<script type="text/javascript">
        function ChangeTab(obj, cat) {
            location.href = '/data/panfile.html?cat='+cat;
        }
</script>
<div class="tit">
<form name="form" action="/data/panfile.html" method="get">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr class="tabtit">
	 ...
	 <td width="18%" class="leftxt">文件名称</td>
	 <td width="5%">文件大小</td>
	 <td width="5%">文件类型</td>
	 <td width="18%" class="leftxt">所属网校</td>
	 <td width="8%">账号</td>
	 <td width="8%">上传人</td>
	 <td width="7%">上传时间</td>
...
<?php if(!empty($panfiles)){
	foreach($panfiles as $key=>$panfile){
?>
    <tr class="<?php echo ($key%2==0)?"tabbg":""?>">
    <td>
	<?php if(empty($panfile['admin_status'])||$panfile['admin_status']==3){?>
    	<input type="checkbox" value="<?php echo $panfile['fileid']?>" name="sel" />
	<?php }?>
	<?php echo max(0,($request['page']-1))*$request['pagesize']+$key+1?></td>
    <td class="leftxt" title="<?php echo $panfile['title']?>"><a class="Jview" href="/data/view.html?fileid=<?php echo $panfile['fileid']?>" title="文件详情" style="text-decoration:underline;color:#075587"><?php  echo ssubstrch($panfile['title'],0,40) ;?></a></td>
	<td><?php echo $panfile['size']?></td>
	<td><?php echo $panfile['suffix']?></td>
	<td class="leftxt"><?php echo $panfile['crname']?></td>
	<td><?php echo $panfile['username']?></td>
	<td><?php echo $panfile['realname']?></td>
	<td><?php echo date("Y-m-d H:i:s",$panfile['dateline'])?></td>
    <td><?php echo $panfile['checkname']? $panfile['checkname']:'--'?></td>
	<td><?php $astatus=$panfile['admin_status']?$panfile['admin_status']:$panfile['teach_status'];if($astatus==1){echo '已通过';}elseif($astatus==2){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
    <?php if($panfile['ispreview']){?>
    <a  target="_blank" href="<?php echo 'http://uppan.ebh.net/kfpreview.html?id='.$panfile['fileid'].'&k='.$panfile['k'];?>" title="文件预览">预览</a>
    <?php } else {?>
    <a  target="_blank" href="<?php echo 'http://uppan.ebh.net/kfatt.html?id='.$panfile['fileid'].'&k='.$panfile['k'];?>" title="文件下载">下载</a>
    <?php }?>
	<a class="Jview" href="/data/view.html?fileid=<?php echo $panfile['fileid']?>" title="文件详情">详情</a>
	<?php if(($panfile['admin_status']==0||$panfile['admin_status']==3) && empty($panfile['teach_status'])){?>
	<a class="Jcheck" toid="<?php echo $panfile['fileid']?>">审核</a>
	<?php }else{?>
        <a class="revoke" toid="<?php echo $panfile['fileid']?>" status="<?php echo $panfile['admin_status']?>">撤销</a>
        <?php }?>
	</td>
...
//                    },2000)
    ...
    $(document).on("click", ".revoke", function () {
       ...
       if (!window.confirm('是否撤销审核')) {
        ...
        var cat = <?php echo $cat;?>;
        $.post('/data/revoke.html', {toid: toid, status: status, type: 11}, function (data) {
            if(cat == -1){
               if (data.code == 0) {
                    $.showmessage({
                        img: 'success',
    ...
    $(document).on("click", ".Jdel", function () {
        ...
        if (confirm("确定要删除该课件吗?")) {
            $.post('/data/delprocess.html', {toid: toid, type: 11}, function () {
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckprocess.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark,type:11},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
