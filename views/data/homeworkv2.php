...
<script type="text/javascript">
        function ChangeTab(obj, cat) {
            location.href = '/data/homeworkv2.html?cat='+cat;
        }
</script>
<form name="form" id="form" action="/data/homeworkv2.html" method="get">
...
<?php foreach($homeworks as $key=>$h){?>
    ...
    <td class="leftxt"><a class="Jview" href="/data/view.html?newexam=1&eid=<?php echo $h['eid']?>" title="作业详情" style="text-decoration:underline;color:#075587" data="<?php echo $h['crid']?>" uid="<?php echo $h['uid']?>"><?php echo $h['esubject']?></a></td>
    ...
    <td><?php echo $h['checkname']?$h['checkname']:'--'?></td>
	 <td ><?php if($h['admin_status']==1 || (empty($h['admin_status'])&&$h['teach_status']==1 )) {echo '已通过';}elseif($h['admin_status']==2 || (empty($h['admin_status'])&&$h['teach_status']==2 )){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
	<a class="Jview" href="/data/view.html?newexam=1&eid=<?php echo $h['eid']?>" title="作业详情">详情</a>
    ...
    $(document).on("click", ".Jcheck", function () {
        ...
        dialog.button([{
            ...
            callback: function () {
                ...
                $.post('/data/checkexam.html', {
                    toid: toid,
                    uid: obj.parent().siblings("td").eq(1).find("a").attr('uid'),
                    subject: obj.parent().siblings("td").eq(1).find("a").text(),
                    crid: obj.parent().siblings("td").eq(1).find("a").attr('data'),
...
//                    },2000)
    ...
    $(document).on("click", ".revoke", function () {
        ...
        var cat = <?php echo $cat;?>;
        $.post('/data/revokeexam.html', {toid: toid, status: status}, function (data) {
            if(cat == -1){
              if (data.code == 0) {
                    $.showmessage({
                        img: 'success',
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckexam.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
