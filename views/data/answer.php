...
<script type="text/javascript">
        function ChangeTab(obj, cat) {
            location.href = '/data/answer.html?cat='+cat;
        }
</script>
<form name="form" id="form" action="/data/answer.html" method="get">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr class="tabtit">
	 ...
	 <td class="leftxt" width="18%">问题标题</td>
	 <td class="leftxt" width="18%">回答内容</td>
	 <td class="leftxt" width="18%">所属网校</td>
	 <td>账号</td>
	 <td>回答者</td>
	 <td>时间</td>
...
<?php foreach($answers as $key=>$answer){?>
    ...
    <td>
	<?php if(empty($answer['admin_status'])||$answer['admin_status']==3){?>
    	<input type="checkbox" value="<?php echo $answer['aid']?>" name="sel" />
	<?php }?>
	<?php echo max(0,($request['page']-1))*$request['pagesize']+$key+1?></td>
    <td class="leftxt" title="<?php echo $answer['title']?>">
    <a class="Jview" href="/data/view.html?aid=<?php echo $answer['aid']?>" title="回答详情" style="text-decoration:underline;color:#075587">
    <?php  echo $answer['title'] ;?></a></td>
    <?php $message =strip_tags($answer['message']); ?>
	<td class="leftxt" title="<?php echo $message?>"><?php  echo $answer['message'] ;?></td>
	<td class="leftxt"><?php echo $answer['crname']?></td>
	<td><?php echo $answer['username']?></td>
	<td><?php echo $answer['realname']?></td>
	<td><?php echo date("Y-m-d H:i:s",$answer['dateline'])?></td>
    <td><?php echo $answer['checkname'] ? $answer['checkname'] : '--'?></td>
	<td><?php $astatus=$answer['admin_status'];if($astatus==1){echo '已通过';}elseif($astatus==2){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
	<a class="Jview" href="/data/view.html?aid=<?php echo $answer['aid']?>" title="回答详情">详情</a>
	<?php if($answer['admin_status']==0||$answer['admin_status']==3){?>
	<a class="Jcheck" toid="<?php echo $answer['aid']?>">审核</a>
    <?php }else{?>
        <a class="revoke" toid="<?php echo $answer['aid']?>" status="<?php echo $answer['admin_status']?>" >撤销</a>
    <?php }?>
	</td>
...
//                    },2000)
    ...
    $(document).on("click", ".revoke", function () {
        ...
        var cat = <?php echo $cat;?>;
        $.post('/data/revoke.html', {toid: toid, status: status, type: 5}, function (data) {
            if(cat == -1){
               if (data.code == 0) {
                    $.showmessage({
                        img: 'success',
    ...
    $(document).on("click", ".Jdel", function () {
        ...
        if (confirm("确定要删除该课件吗?")) {
            $.post('/data/delprocess.html', {toid: toid, type: 5}, function () {
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckprocess.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark,type:5},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
