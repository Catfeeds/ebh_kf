...
<body>
<?php show_dialog(".Jimg","Jimg","800","600",true,false); ?>
...
<script type="text/javascript">
        function ChangeTab(obj, cat) {
            location.href = '/data/attachment.html?cat='+cat;
        }
</script>
<form name="form" id="form" action="/data/attachment.html" method="get">
...
<input value="<?php echo $request["crname"]?>" class="inp wid140" readonly id="crname" name="crname"/><button type="button" id="dialog-link" class="comcheck">选择</button>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="word-break:break-all;">
  <tr class="tabtit">
     ...
     <td class="leftxt" width="15%">附件名称</td>
     <td class="leftxt" width="12%">所属课件</td>
     <td width="8%">账号</td>
     <td width="5%">原作者</td>
     <td class="leftxt" width="10%">所属网校</td>
     <td width="7%">所属分类</td>
     <td width="10%">添加时间</td>
     <td width="3%">格式</td>
     <td width="3%">大小</td>
...
<?php foreach($attachments as $key=>$attach){?>
    <tr class="<?php echo ($key%2==0)?"tabbg":""?>" param="<?php echo $attach['uid'].'_'.$attach['crid'].'_'.$attach['folderid']?>" id="tr_index_<?php echo $key?>">
    <td>
	<?php if(empty($attach['admin_status'])||$attach['admin_status']==3){?>
    	<input type="checkbox" value="<?php echo $attach['attid']?>" name="sel" />
	<?php }?>
    ...
    <a href='javascript:play("<?php echo urlencode($attach['source'].'kfattach.html?attid='.$attach['attid'].'&k='.$attach['k'].'|'.$attach['title']);?>")' style="text-decoration:underline;color:#075587"><?php echo $attach['title']?></a>
    <?php }elseif(in_array($attach['suffix'],array('swf','flv','mp3'))){?>
    <a class="Jplay" href="/play.html?attid=<?php echo $attach['attid']?>" title="<?php echo $attach['title']?>" style="text-decoration:underline;color:#075587"><?php echo $attach['title']?></a>
    <?php }elseif(!empty($attach['url'])){?>
    <a  target="_blank" href="<?php echo  $attach['source'].'kfattach.html?attid='.$attach['attid'].'&k='.$attach['k'];?>" title="附件下载" style="text-decoration:underline;color:#075587"><?php echo $attach['title']?></a>
    <?php }else{?>
        <a title="无内容" style="text-decoration:underline;color:#075587"><?php echo $attach['title']?></a>
    <?php }?>
    </td>
    <td class="leftxt" ><?php
    $zhibo = ($attach['islive']==1) ? '<i class="label-live" title="直播课件">直播</i>': '';
    echo $zhibo;
    ?>
    <?php echo $attach['cwtitle']?></td>
    <td><?php echo $attach['username']?></td>
    <td><?php echo $attach['realname']?></td>
    <td class="leftxt"><?php echo $attach['crname']?></td>
    <td><?php echo $attach['foldername']?></td>
    <td><?php echo date("Y-m-d H:i:s",$attach['dateline'])?></td>
    <td><?php echo $attach['suffix']?></td>
    <td><?php echo getSize($attach['size'])?></td>
    <td><?php echo $attach['checkname'] ? $attach['checkname'] : '--'?></td>
    <td ><?php if($attach['admin_status']==1 || (empty($attach['admin_status'])&&$attach['teach_status']==1 )) {echo '已通过';}elseif($attach['admin_status']==2 || (empty($attach['admin_status'])&&$attach['teach_status']==2 )){ echo '未通过';}else{echo '未审核';}?></td>
    <td class="tablink">
    <?php
    if(preg_match("/.*(\.ebh|\.ebhp)$/",$attach['url'])){ ?>
    <a href='javascript:play("<?php echo urlencode($attach['source'].'kfattach.html?attid='.$attach['attid'].'&k='.$attach['k'].'|'.$attach['title']);?>")'>播放</a>
    <?php }elseif(in_array($attach['suffix'],array('swf','flv','mp3','avi'))){?>
    <a class="Jplay" href="/play.html?attid=<?php echo $attach['attid']?>" title="<?php echo $attach['title']?>">播放</a>
    <?php }elseif(in_array($attach['suffix'],array('jpg','jpeg','png','gif'))){?>
        <a class="Jimg" href="/data/viewImage.html?attid=<?php echo $attach['attid']?>" title="<?php echo $attach['title']?>">预览</a>
    <?php }elseif(!empty($attach['url'])){?>
    <a  target="_blank" href="<?php echo  $attach['source'].'kfattach.html?attid='.$attach['attid'].'&k='.$attach['k'];?>" title="附件下载">下载</a>
    <?php }else{?>
        <label style="color: #075587">无内容</label>
        <?php }?>
    <a class="Jview" href="/data/view.html?attid=<?php echo $attach['attid']?>" title="附件详情">详情</a>
    <?php if(($attach['admin_status']==0||$attach['admin_status']==3) && empty($attach['teach_status'])){?>
    <a class="Jcheck" toid="<?php echo $attach['attid']?>">审核</a>
    <?php }else{?>
        <a class="revoke" toid="<?php echo $attach['attid']?>" status="<?php echo empty($attach['teach_status'])?$attach['admin_status']:$attach['teach_status'];?>" >撤销</a>
        <?php }?>
    </td>
</tr>
 <?php }?> 
...
//                    },2000)
    ...
    $(document).on("click", ".revoke", function () {
        ...
        numarr = num.split('_');
        var cat = <?php echo $cat;?>;
        $.post('/data/revoke.html', {toid: toid, status: status, type: 2}, function (data) {
            if(cat == -1){
                if (data.code == 0) {
                    $.showmessage({
                        img: 'success',
                    ...
                    var check = "<input value="+toid+" name='sel' type='checkbox'>"+(parseInt(numarr[2])+1);
    ...
    $(document).on("click", ".Jdel", function () {
        ...
        if (confirm("确定要删除该课件吗?")) {
            $.post('/data/delprocess.html', {toid: toid, type: 2}, function () {
	...
	$("#docheck").click(function(){
		...
		dialog.button([{
	        ...
	        callback: function () {
		        ...
		        //alert(selectrole); return false; 
		        $.post('/data/multcheckprocess.html',{ids:idarr.join(","),admin_status:admin_status,admin_remark:remark,type:2},function(data){
			        if(data.msg){
				        //alert(data.msg);
                        $.showmessage({
                            img : 'success',
