...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td><input type="text" name="rolename" class="inp a40" value="<?=$role['rolename']?>"  id="rolename" /><span style="color:red;"> *</span></td>
  </tr>
  <tr>
    ...
    <td>
      ...
      <?php foreach($modulelist as $key => $value) {?>
      	<li><label><input type="checkbox" name="access[]" value="<?=$key?>" <?php if(strpos(','.$role['access'].',', ','.$key.',') !== false) echo 'checked="checked" '; ?> /> <?=$value?></label></li>
      <?php }?>
      </ul>
    </td>
  </tr>   
  <tr>
    ...
    <td>
    所有学校:<input type="radio" name="rangetype" value="0" <?php if($role['rangetype'] == 0) echo'checked="checked" '?>/><br />
    部分学校:<input type="radio" name="rangetype" value="1" <?php if($role['rangetype'] == 1) echo'checked="checked" '?>/> <button type="button" id="dialog-link" class="ctbtn">添加学校</button>
      <ul id="classroom_range" class="ctlist clearfix" style="margin-top:3px;">
      <?php if (!empty($role['classroom_list'])){ foreach ($role['classroom_list'] as $value) {?>
	  	<li id="crli_<?=$value['crid']?>"><label><input type="checkbox" name="classroom[]" value="<?=$value['crid']?>" checked="checked" /> <?=$value['crname']?></label></li>
	  <?php }}?>
      </ul>
    </td>    
  </tr>
  <tr>
    ...
    <td><textarea name="remark" class="inp a60" style="height:76px;"><?=$role['remark']?></textarea></td>    
  </tr>  
 </table>
</div>
<div style="text-align:center;margin-top:50px"><input type="submit" value="修改" class="combtn cbtn_4 formsbt"   /></div>
</form>
